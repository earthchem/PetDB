...
public class SampleInfoRockDCtlQuery extends NonPersistentDynamicCtlQuery 
{

	public SampleInfoRockDCtlQuery(String filter)
        {
                super(filter);
        }
	...
	protected int setFilter()
	{
		qry = "select distinct an.analysis_num, p.last_name||decode(nvl(r.pub_year,''),'','',', '||r.pub_year),"
			...
			+ " mat.material_name, it.item_type_code, im.item_code, c.value_meas, an.analysis_comment"
			+ " from person p, author_list al, reference r, method m, data_quality dq,"
			+ " sample_comment sc, analysis an, chemistry c, item_type it,"
			+ " itemtype_list itl, item_measured im, material mat, batch b"
			...
			+ " and mat.material_num = b.material_num"
			+ " and b.sample_num = " + v_filter
			+ " and b.material_num in (3,4,7,8)"
			+ " order by an.analysis_num, p.last_name||decode(nvl(r.pub_year,''),'','',', '||r.pub_year),"
			+ " sc.alias, m.method_code,"
			+ " mat.material_name, it.item_type_code";
		//DataDCtlQuery.writeQueryToFile(qry,"C:\\Users\\Lulin Song\\Downloads\\sampleInfoRockDqry.txt");
		return 1;
	} 	  
}
