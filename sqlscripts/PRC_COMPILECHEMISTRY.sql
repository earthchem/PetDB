...
-- Start Trace Section
   ...
   commit;
   --update null values
   for curBC in (select distinct data_quality_num from BF_COMPILECHEM)
   loop
     select m.method_num, m.method_code into nMethodnum, cMethodCode
     from method m, data_quality dq
     where m.method_num = dq.method_num
       and dq.data_quality_num = curBC.data_quality_num;
     update BF_COMPILECHEM b
     set b.method_num = nMethodnum,
         b.method_code = cMethodCode
     where b.data_quality_num = curBC.data_quality_num;
   end loop;
   commit;
   ...
   set t.methodprominence = to_number(decode(method_code, 'MS-ID',8,'MS',7, 'ICPMS',6,'SSMS',5,'XRF',4,'DCP',3,'INAA',2,'AAS',1,0))*10000 + nvl(pub_year,0);
   ...
   SELECT a.sample_num, a.item_measured_num, a.item_type, a.material_num, 
          a.data_quality_num, a.ref_num, a.method_num, a.pub_year,
   ...
   INSERT INTO BF_COMPILECHEM (sample_num, item_measured_num, material_num, item_type,
                                     data_quality_num, ref_num, method_num, method_code,
                                     value_meas, unit, stdev, stdev_type, data_etnered_date)
   ...
   -- Select best data based on method used, then year published
   update BF_COMPILECHEM t
   set t.methodprominence = to_number(decode (t.method_code, 'XRF',6,'DCP',5,'WET',4,'EMP',3,'ES',2,'CALC',1,0))*10000 + nvl(t.pub_year,0);
   ...
   SELECT b.sample_num, c.item_measured_num, it.item_type_code, m.material_num,
            a.data_quality_num, null , null, null,
            c.value_meas, c.unit, c.stdev, c.stdev_type, c.data_entered_date
   ...
   --Update data for missing columns
   update BF_COMPILECHEM b
   set (b.method_num, b.method_code, b.ref_num, b.pub_year) =
       (select m.method_num, m.method_code, r.ref_num, r.pub_year
   ...
   SELECT a.sample_num, a.item_measured_num, a.item_type, a.material_num, 
          ...
          a.value_meas, a.unit, a.stdev, a.stdev_type, a.data_etnered_date
   FROM BF_COMPILECHEM a
   WHERE to_number(decode (a.method_code, 'MS-ID',8,'MS',7, 'ICPMS',6,'DCP',5,'SSMS',4,'INAA',3,'NAA',2,0))*10000 + nvl(a.pub_year,0) =
         (SELECT max(to_number(decode(method_code, 'MS-ID',8,'MS',7, 'ICPMS',6,'DCP',5,'SSMS',4,'INAA',3,'NAA',2,0))*10000 + nvl(pub_year,0))
          FROM BF_COMPILECHEM
          WHERE sample_num        = a.sample_num
            AND item_measured_num = a.item_measured_num
            AND material_num      = a.material_num);   
   ...
   SELECT a.sample_num, a.item_measured_num, a.item_type, a.material_num, 
          ...
          a.value_meas, a.unit, a.stdev, a.stdev_type, a.data_etnered_date
   FROM BF_COMPILECHEM a
   WHERE to_number(decode (a.method_code, 'MS',8,'ANC',7,'ISE',6,'INAA',5,'EMP',4,'NN',3,0))*10000 + nvl(a.pub_year,0) =
         (SELECT max(to_number(decode(method_code, 'MS',8,'ANC',7,'ISE',6,'INAA',5,'EMP',4,'NN',3,0))*10000 + nvl(pub_year,0))
          FROM BF_COMPILECHEM
          WHERE sample_num        = a.sample_num
            AND item_measured_num = a.item_measured_num
            AND material_num      = a.material_num);   
   ...
   SELECT a.sample_num, a.item_measured_num, a.item_type, a.material_num, 
          ...
          a.value_meas, a.unit, a.stdev, a.stdev_type, a.data_etnered_date
   FROM BF_COMPILECHEM a
   WHERE  nvl(a.pub_year,0)*100000 - nvl(a.stdev, 100000)  =
         (SELECT max(nvl(pub_year,0)*100000 - nvl(a.stdev, 100000))
          FROM BF_COMPILECHEM
          WHERE sample_num        = a.sample_num
            AND item_measured_num = a.item_measured_num
            AND material_num      = a.material_num);   
